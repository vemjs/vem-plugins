---
"@vemjs/plugin-autopairs": patch
"@vemjs/plugin-git": patch
---

autopairs: typing a closing bracket/quote that's already sitting right after the cursor (typically the one autopairs just inserted) now steps over it instead of leaving a duplicate (`())` → `()`).
