  ...
  activate(context) {
    ...
    const updateStatusline = () => {
      ...
      try {
        ...
        // Plain text, no icon glyph: the web build only loads plain
        // "JetBrains Mono" (Google Fonts), which has none of the Nerd Font
        // Private Use Area glyphs a patched font would provide, so an icon
        // codepoint here renders as a tofu box for every browser visitor.
        const branchSeg = {
          text: " main ",
          bg: "#334155", // slate-700
          color: "#e2e8f0",
        ...
        // Standard Unicode symbols (not Nerd Font PUA glyphs, which the
        // plain web-loaded JetBrains Mono has no coverage for).
        let diagText = "";
