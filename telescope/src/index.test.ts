...
describe("Telescope openFile capability", () => {
  it("routes find-files selection through the host openFile when provided", () => {
    const editor = new VemEditorState("");
    editor.projectFiles = ["src/main.ts", "README.md"];
    const opened: string[] = [];
    const registry = new PluginRegistry(editor, {
      openFile: (path) => {
        opened.push(path);
    ...
    registry.executeCommand("telescope.findFiles");
    editor.handleKey("Enter"); // select the first item
